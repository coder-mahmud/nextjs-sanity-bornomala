...
const ResultsHero = async () => {
  const siteOptions = await getSiteOpions();
  const resultPageImage = siteOptions?.resultPageImage?.node?.sourceUrl


  return (
    <section  data-aos="fade-up" data-aos-offset="0" data-aos-duration="1000" data-aos-delay={0} className='bg-cover bg-center bg-no-repeat' style={{ backgroundImage: `url('${resultPageImage}')` }}>
